...
var users, meditations, communityUsers, seelenjournalKlientinnen, seelenjournalPdfs;
var init_schema = __esm({
  "drizzle/schema.ts"() {
    ...
    seelenjournalKlientinnen = mysqlTable("seelenjournal_klientinnen", {
      id: int("id").autoincrement().primaryKey(),
      name: varchar("name", { length: 255 }).notNull(),
      email: varchar("email", { length: 320 }).notNull().unique(),
    ...
    seelenjournalPdfs = mysqlTable("seelenjournal_pdfs", {
      id: int("id").autoincrement().primaryKey(),
...
__export(db_exports, {
  ...
  createSeelenjournalKlientin: () => createSeelenjournalKlientin,
  createSeelenjournalPdf: () => createSeelenjournalPdf,
  deleteCommunityUser: () => deleteCommunityUser,
  deleteMeditation: () => deleteMeditation,
  deleteSeelenjournalKlientin: () => deleteSeelenjournalKlientin,
  deleteSeelenjournalPdf: () => deleteSeelenjournalPdf,
  getActiveMeditations: () => getActiveMeditations,
  getAllCommunityUsers: () => getAllCommunityUsers,
  getAllMeditations: () => getAllMeditations,
  getAllSeelenjournalKlientinnen: () => getAllSeelenjournalKlientinnen,
  getCommunityUserByEmail: () => getCommunityUserByEmail,
  getDb: () => getDb,
  getSeelenjournalKlientinByEmail: () => getSeelenjournalKlientinByEmail,
  getSeelenjournalPdfs: () => getSeelenjournalPdfs,
...
async function getAllSeelenjournalKlientinnen() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(seelenjournalKlientinnen).orderBy(desc(seelenjournalKlientinnen.createdAt));
}
async function getSeelenjournalKlientinByEmail(email) {
  const db = await getDb();
  if (!db) return void 0;
...
async function createSeelenjournalKlientin(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function deleteSeelenjournalKlientin(id) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function getSeelenjournalPdfs(klientinId) {
  const db = await getDb();
  if (!db) return [];
...
async function createSeelenjournalPdf(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(seelenjournalPdfs).values(data);
  return result[0].insertId;
}
async function deleteSeelenjournalPdf(id) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
var appRouter = router({
  ...
  seelenjournal: router({
    listKlientinnen: publicProcedure.query(async () => {
      return getAllSeelenjournalKlientinnen();
    }),
    createKlientin: publicProcedure.input(z2.object({
      name: z2.string().min(1),
      email: z2.string().email(),
      notizen: z2.string().optional()
    })).mutation(async ({ input }) => {
      const existing = await getSeelenjournalKlientinByEmail(input.email);
      if (existing) return { success: false, error: "exists" };
      const id = await createSeelenjournalKlientin(input);
      return { success: true, id };
    }),
    deleteKlientin: publicProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ input }) => {
      await deleteSeelenjournalKlientin(input.id);
      return { success: true };
    }),
    listPdfs: publicProcedure.input(z2.object({ klientinId: z2.number() })).mutation(async ({ input }) => {
      return getSeelenjournalPdfs(input.klientinId);
    }),
    deletePdf: publicProcedure.input(z2.object({ id: z2.number() })).mutation(async ({ input }) => {
      await deleteSeelenjournalPdf(input.id);
      return { success: true };
    }),
    createPdf: publicProcedure.input(z2.object({
      klientinId: z2.number(),
      titel: z2.string().min(1),
      pdfUrl: z2.string().min(1),
      fileName: z2.string().min(1)
    })).mutation(async ({ input }) => {
      const id = await createSeelenjournalPdf(input);
      return { success: true, id };
    })
  }),
  storage: router({
...
async function startServer() {
  ...
  app.post("/api/upload-pdf", upload.single("file"), async (req, res) => {
    try {
      ...
      const { createSeelenjournalPdf: createSeelenjournalPdf2 } = await Promise.resolve().then(() => (init_db(), db_exports));
      const pdfId = await createSeelenjournalPdf2({
        klientinId,
        titel,
        pdfUrl: result.url,
